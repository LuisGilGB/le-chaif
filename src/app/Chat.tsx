        ...
        >
          {conversation.map((message: ClientMessage) =>
            message.role === 'user' ? (
              <div
                key={message.id}
                className="bg-amber-200 rounded-b-xl rounded-tl-xl text-right self-right px-4 py-2 ml-auto"
              >
                {message.display}
              </div>
            ) : (
              <div key={message.id} className="bg-sky-200 rounded-b-xl rounded-tr-xl px-4 py-2 mr-auto">
                {message.display}
              </div>
            ),
          )}
